...
URIFASaveGame::URIFASaveGame()
{
	...
	static ConstructorHelpers::FObjectFinder<USkeletalMesh> HairPart(TEXT("/Script/Engine.SkeletalMesh'/Game/RifaCharacters/idletest_hair01.idletest_hair01'"));
	if (HairPart.Succeeded()) 
	{
		CurrentHairPart = HairPart.Object;
	}

	static ConstructorHelpers::FObjectFinder<UMaterialInterface> CharacterMaterial(TEXT("/Script/Engine.Material'/Game/RifaCharacters/Texture/Default.Default'"));
	if (CharacterMaterial.Succeeded()) 
	{
		CurrentCharacterMaterial = CharacterMaterial.Object;
	}
}
